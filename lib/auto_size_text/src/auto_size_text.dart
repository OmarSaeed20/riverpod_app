...
class AutoSizeText extends StatefulWidget {
  ...
  /// The text to display.
  ///
  /// This will be null if a [_textSpan] is provided instead.
  final String? data;
  ...
  final TextScaler? _textScaler; // Changed from textScaleFactor
...
class AutoSizeTextState extends State<AutoSizeText> {
  ...
  List _calculateFontSize(
    ...
    var span = TextSpan(
      style: widget._textSpan?.style ?? style,
      text: widget._textSpan?.text ?? widget.data,
      children: widget._textSpan?.children,
      recognizer: widget._textSpan?.recognizer,
    );

    // Updated to use textScaler
    final textScaler = widget._textScaler ?? MediaQuery.textScalerOf(context);
    final userScale = textScaler.scale(1); // Get linear scale factor

    int left;
    int right;

    var presetFontSizes = widget.presetFontSizes?.reversed.toList();
    if (presetFontSizes == null && style?.fontSize != null) {
      num defaultFontSize = style!.fontSize!.clamp(
        widget.minFontSize,
        widget.maxFontSize,
      );
  ...
  bool _checkTextFits(
    ...
    if (!widget.wrapWords) {
      ...
      var wordWrapTp = TextPainter(
        ...
        textScaler: TextScaler.linear(scale), // Updated from textScaleFactor
  ...
  Widget _buildText(double fontSize, TextStyle? style, int? maxLines) {
    if (widget.data != null) {
      return Text(
        ...
        textScaler: TextScaler.linear(1), // Updated from textScaleFactor: 1
        maxLines: maxLines,
